...
const initState = {
    postList: postItemInit,
    editPost: null,
};
export const addPost = createAction('addPost');
export const delPost = createAction('delPost');
export const editPost = createAction('turnEditMode');
export const cancelEditingMode = createAction('cancelEditingMode');
export const finishedEditing = createAction('finishedEditing');

const blogReducer = createReducer(initState, (builder) => {
    builder.addCase(addPost, (state, action) => {
        state.postList.push(action.payload);
    });
    builder.addCase(delPost, (state, action) => {
    ...
    builder.addCase(editPost, (state, action) => {
        const postEdited = state.postList.find((post) => post.id === action.payload);
        state.editPost = postEdited;
    });
    builder.addCase(cancelEditingMode, (state) => {
        state.editPost = null;
    });
    builder.addCase(finishedEditing, (state, action) => {
